...
var PACKET_TYPE_MESSAGE = 10001;
var PACKET_TYPE_AUTH = 10002;
var PACKET_TYPE_PING = 10003;
var PACKET_TYPE_PONG = 10004;
var PACKET_TYPE_TIMEOUT = 10005;
var PACKET_TYPE_CLOSE = 10006;
var PACKET_TYPE_SYSTEM_ = 10007;
var PACKET_TYPE_ERROR_ = 10008;
...
function Tunnel() {
  ...
  function registerEventHandler(eventType, eventHandler) {
    // console.log('eventType, eventHandler', eventType, eventHandler);
  ...
  function dispatchEvent(eventType, eventPayload) {
    // console.log('dispatchEvent : {} , {}', eventType, eventPayload);
  ...
  function emitMessagePacket(messageType, messageContent) {
     ...
     * 发送验证包
     */
  function emitAuthPacket(messageType, messageContent) {
    var packet = {
  ...
  function resolvePacket(raw) {
    ...
    switch (packet.code) {
      case PACKET_TYPE_MESSAGE:
        handleMessagePacket(packet.extend.type, packet.body);
        break;
     ...
     */
  function handleMessagePacket(type, content) {
    dispatchEscapedEvent(type, content);
  }
     ...
     * 该值将在与信道服务器建立连接后被更新
     */
  let pingPongTimeout = 90000;
  let pingPongSend = 15000;
  let pingTimer = 0;
  let pongTimer = 0;
  ...
  function nextPing() {
    ...
    console.log('set ping');
    pingTimer = setTimeout(ping, pingPongSend);
  }
